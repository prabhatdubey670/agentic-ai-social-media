...
from config import (DAILY_LIMITS, DELAYS, TARGET_TOPICS, TELEGRAM_BOT_TOKEN,
                    ...
                    LINKEDIN_EMAIL, LINKEDIN_PASSWORD, KAFKA,
                    DRY_RUN_DEFAULT, BROWSER_HEADLESS, REQUIRE_MANUAL_APPROVAL)
...
class XAgent:
    ...
    async def login(self):
        if not X_USERNAME or not X_PASSWORD:
            raise RuntimeError("X_USERNAME and X_PASSWORD must be set for X automation")
    ...
    async def like(self, article) -> bool:
        if self.dry_run:
            print("DRY RUN: would like X post")
            return False
        try:
            btn = await article.query_selector('[data-testid="like"]')
            if btn:
                await btn.click()
    ...
    async def comment(self, article, text: str) -> bool:
        if self.dry_run:
            print(f"DRY RUN: would comment on X: {text[:100]}")
            return False
        try:
            btn = await article.query_selector('[data-testid="reply"]')
            if btn:
                await btn.click()
                await asyncio.sleep(2)
...
class LinkedInAgent:
    ...
    async def login(self):
        if not LINKEDIN_EMAIL or not LINKEDIN_PASSWORD:
            raise RuntimeError("LINKEDIN_EMAIL and LINKEDIN_PASSWORD must be set for LinkedIn automation")
    ...
    if mode in ["post", "engage", "full"]:
        async with async_playwright() as p:
            ...
            if mode in ["post", "engage", "full"]:
                ...
                x_agent = XAgent(x_page, llm, db, content_gen, notifier)
                await x_agent.login()

                if mode in ["post", "full"]:
                    print("\n📝 Generating X.com posts...")
            ...
            if mode in ["engage", "full"]:
                ...
                li_agent = LinkedInAgent(li_page, llm, db, content_gen, notifier)
...
async def main(mode: str = "draft", dry_run: bool = True, approve_id: str = "",
    ...
    if mode == "post" and dry_run:
        build_drafts(db, content_gen, x_count=DAILY_LIMITS["x"]["posts"], linkedin_count=0)

    elif mode in ["post", "engage", "full"]:
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=BROWSER_HEADLESS)

            if mode in ["post", "engage", "full"]:
                x_page = await browser.new_page()
                x_agent = XAgent(x_page, llm, db, content_gen, notifier, dry_run=dry_run)
                await x_agent.login()
            ...
            if mode in ["engage", "full"]:
                li_page = await browser.new_page()
                li_agent = LinkedInAgent(li_page, llm, db, content_gen, notifier, dry_run=dry_run)
                await li_agent.login()
                print("\nEngaging on LinkedIn...")
                await li_agent.run_engagement()
